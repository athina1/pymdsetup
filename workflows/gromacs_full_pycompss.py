...
@task(dependency_file_in=FILE_IN, dependency_file_out=FILE_OUT, task_path=IN,
      ...
      output_pdb_path=IN,
      mutation=IN,
      log_path=IN, error_path=IN, scwrl_path=IN)
def scwrlPyCOMPSs(dependency_file_in, dependency_file_out, task_path,
                  pdb_path,
                  output_pdb_path,
                  mutation,
                  log_path, error_path, scwrl_path):
    """ Launches SCWRL 4 using the PyCOMPSs library."""
    fu.create_change_dir(task_path)

    scwrl.Scwrl4(pdb_path,
                 output_pdb_path,
                 mutation,
                 log_path, error_path, scwrl_path).launch()
